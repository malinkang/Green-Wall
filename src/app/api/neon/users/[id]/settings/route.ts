...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const rows = await neonSql`SELECT * FROM user_settings WHERE user_id = ${id}`
  ...
  try {
    ...
    const rows = await neonSql`
      INSERT INTO user_settings (
        ...
        days_label, show_attribution, show_safari_header, show_header, show_card, year_order, year_start, year_end, updated_at
      ) VALUES (
        ...
        ${show_safari_header ?? null}, ${show_header ?? null}, ${show_card ?? null}, ${year_order ?? null}, ${year_start ?? null}, ${year_end ?? null}, NOW()
      )
      ON CONFLICT (user_id)
      DO UPDATE SET
        unit = EXCLUDED.unit,
